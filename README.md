...
- **주요 고객층 분석 (누가 가장 많이 살까?)**: 20대 여성 주요 고객층 및 연령대/성별 소비 비중
- **요일별 매출 분석 (어느 요일이 강할까?)**: 주중 평균 대비 금요일(+21%) 등 피크 요일 시각화
- **경쟁 강도 & 종합 점수 (경쟁은 어떨까? / Overall Insight)**: 동일 업종 점포수(134개, +12개), 경쟁 강도(매우 높음), 종합 탐색 점수(SCORE 82/100) 및 데이터 테이크어웨이
